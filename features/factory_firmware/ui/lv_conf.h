 ...
 * @file lv_conf.h
 *
 * @brief LVGL configuration parameters
 *
...
#  define LV_MEM_SIZE           DEMO_GUI_HEAP_SIZE    /*[bytes]*/
...
#if LV_TICK_CUSTOM
#define LV_TICK_CUSTOM_INCLUDE  "gdi.h"         /*Header for the system time function*/
#define LV_TICK_CUSTOM_SYS_TIME_EXPR (gdi_convert_ticks_to_us(gdi_get_sys_uptime_ticks()) / 1000)     /*Expression evaluating to current system time in ms*/
...
#define DLG_LVGL_USE_GPU_DA1470X 1
#if DLG_LVGL_USE_GPU_DA1470X
#  define DLG_LVGL_GPU_DA1470X_INCLUDE_PATH "dave_driver.h"

/* Maximum size of temporary buffer used by the GPU to perform BLIT operations with mask. The size of
 * the buffer affects the number of blocks required for a BLIT operation and hence the performance of
 * the operation. A minimum value of 20K is recommended */
#  define DLG_LVGL_GPU_BLIT_MASK_BUFFER_SIZE    (20 * 1024)

/* Set the low limit in pixels that a BLIT operation with mask is performed by the GPU. SW is faster
 * in smaller areas whereas the GPU is faster in larger areas. Recommended value of 2000 */
#  define DLG_LVGL_GPU_BLIT_MASK_SIZE_LIMIT     2000
#endif
...
#define LV_ASSERT_HANDLER_INCLUDE       "osal.h"
#define LV_ASSERT_HANDLER               OS_ASSERT(0);   /*Halt by default*/
