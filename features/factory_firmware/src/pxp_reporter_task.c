...
static void led_toggle(void)
{
        ...
        if (hw_led_get_states() == HW_LED_MSK_LED_2) {
...
static void do_alert(uint8_t level)
{
        ...
        case 2:
                ...
                hw_led_pwm_set_duty_cycle_pct_off(HW_LED_ID_LED_2, 10000, 0); // Duty cycle = 100%
                hw_led_pwm_set_frequency_hz(1000); // Frequency = 1 kHz
                /* Enable LED2 and start timer for blinking */
                hw_led_on(HW_LED_MSK_LED_2);
...
OS_TASK_FUNCTION(pxp_reporter_task, params)
{
