...
#if (DLG_LVGL_CF == 1)
    else if(dsc->header.cf == LV_IMG_CF_ARGB8888) {
        uint32_t px = (dsc->header.w * y + x)*4;
        p_color.ch.blue  = buf_u8[px + 0];
        p_color.ch.green = buf_u8[px + 1];
        p_color.ch.red   = buf_u8[px + 2];
    }
    else if(dsc->header.cf == LV_IMG_CF_ARGB4444) {
        uint32_t px = (dsc->header.w * y + x)*2;
        p_color.ch.blue  = (( buf_u8[px]     & 0x0F)       * 0x1F) / 0x0F;
        p_color.ch.green = (((buf_u8[px]     & 0xF0) >> 4) * 0x1F) / 0x0F;
        p_color.ch.red   = (( buf_u8[px + 1] & 0x0F)       * 0x1F) / 0x0F;
    }
    else if(dsc->header.cf == LV_IMG_CF_ARGB1555) {
        uint32_t px = (dsc->header.w * y + x)*2;
        p_color.ch.blue  =   (buf_u8[px]     & 0x1F);
        p_color.ch.green = (((buf_u8[px]     & 0xE0) >> 5) | ((buf_u8[px + 1] & 0x3) << 3) * 0x3F) / 0x1F;
        p_color.ch.red   =   (buf_u8[px + 1] & 0x7C) >> 2;
    }
    else if(dsc->header.cf == LV_IMG_CF_RGBA8888) {
        uint32_t px = (dsc->header.w * y + x)*4;
        p_color.ch.blue  = buf_u8[px + 1];
        p_color.ch.green = buf_u8[px + 2];
        p_color.ch.red   = buf_u8[px + 3];
    }
    else if(dsc->header.cf == LV_IMG_CF_RGBA4444) {
        uint32_t px = (dsc->header.w * y + x)*2;
        p_color.ch.blue  = (((buf_u8[px]     & 0xF0) >> 4) * 0x1F) / 0x0F;
        p_color.ch.green = (( buf_u8[px + 1] & 0x0F)       * 0x1F) / 0x0F;
        p_color.ch.red   = (((buf_u8[px + 1] & 0xF0) >> 4) * 0x1F) / 0x0F;
    }
    else if(dsc->header.cf == LV_IMG_CF_RGBA5551) {
        uint32_t px = (dsc->header.w * y + x)*2;
        p_color.ch.blue  =   (buf_u8[px]     & 0x3E) >> 1;
        p_color.ch.green = (((buf_u8[px]     & 0xC0) >> 6) | ((buf_u8[px + 1] & 0x7) << 3) * 0x3F) / 0x1F;
        p_color.ch.red   =   (buf_u8[px + 1] & 0xF8) >> 3;
    }
    else if(dsc->header.cf == LV_IMG_CF_RGB888) {
        uint32_t px = (dsc->header.w * y + x)*4;
        p_color.ch.blue  = buf_u8[px + 0];
        p_color.ch.green = buf_u8[px + 1];
        p_color.ch.red   = buf_u8[px + 2];
    }
    else if(dsc->header.cf == LV_IMG_CF_AI44) {
        buf_u8 += sizeof(lv_color32_t) * 16; /*Skip the palette*/
        /*Get the current pixel.
         *dsc->header.w + 1 means rounding up to 2 because the lines are byte aligned
         *so the possible real width are 2 ,4, 6 ...*/
        uint32_t px = dsc->header.w * y + x;
        p_color.full = buf_u8[px] & 0x0F;
    }
#endif /* DLG_LVGL_CF */
...
lv_opa_t lv_img_buf_get_px_alpha(lv_img_dsc_t * dsc, lv_coord_t x, lv_coord_t y)
{
    ...
    if(dsc->header.cf == LV_IMG_CF_TRUE_COLOR_ALPHA) {
        ...
        uint32_t px = (dsc->header.w * y + x)*2;
        return ((buf_u8[px + 1] & 0xF0)>>4);
    }
    else if(dsc->header.cf == LV_IMG_CF_ARGB1555) {
        /*opa -> [0,1]*/
        uint32_t px = (dsc->header.w * y + x)*2;
        return ((buf_u8[px + 1] & 0x80) == 0x80);
    }
    else if(dsc->header.cf == LV_IMG_CF_RGBA8888) {
        uint32_t px = (dsc->header.w * y + x)*4;
        return buf_u8[px + 0];
        ...
        uint32_t px = (dsc->header.w * y + x)*2;
        return (buf_u8[px] & 0xF);
    }
    else if(dsc->header.cf == LV_IMG_CF_RGBA5551) {
        uint32_t px = (dsc->header.w * y + x)*2;
        return (buf_u8[px] & 0x1);
    }
    else if(dsc->header.cf == LV_IMG_CF_AI44) {
        buf_u8 += sizeof(lv_color32_t) * 16; /*Skip the palette*/
        /*Get the current pixel.
...
void lv_img_buf_set_px_alpha(lv_img_dsc_t * dsc, lv_coord_t x, lv_coord_t y, lv_opa_t opa)
{
    ...
    if(dsc->header.cf == LV_IMG_CF_TRUE_COLOR_ALPHA) {
        ...
        uint32_t px = (dsc->header.w * y + x)*4;
        buf_u8[px + 3]  = opa; /* alpha */
    }
    else if(dsc->header.cf == LV_IMG_CF_ARGB4444) {
        uint32_t px = (dsc->header.w * y + x)*2;
        buf_u8[px + 1] = (buf_u8[px + 1] & 0x0F) | (opa & 0xF0);
    }
    else if(dsc->header.cf == LV_IMG_CF_ARGB1555) {
        /*opa -> [0,1]*/
        uint32_t px = (dsc->header.w * y + x)*2;
        buf_u8[px + 1] = (buf_u8[px + 1] & 0x7F) | (opa & 0x80);
    }
    else if(dsc->header.cf == LV_IMG_CF_RGBA8888) {
        uint32_t px = (dsc->header.w * y + x)*4;
        buf_u8[px + 0]  = opa; /* alpha */
    }
    else if(dsc->header.cf == LV_IMG_CF_RGBA4444) {
        uint32_t px = (dsc->header.w * y + x)*2;
        buf_u8[px]     = (buf_u8[px] & 0xF0) | (opa>>4);
    }
    else if(dsc->header.cf == LV_IMG_CF_RGBA5551) {
        uint32_t px = (dsc->header.w * y + x)*2;
        buf_u8[px]     = (buf_u8[px] & 0xFE) | (opa>>7);
    }
    else if(dsc->header.cf == LV_IMG_CF_AI44) {
        buf_u8 += sizeof(lv_color32_t) * 16; /*Skip the palette*/
        /*Get the current pixel.
...
void lv_img_buf_set_px_color(lv_img_dsc_t * dsc, lv_coord_t x, lv_coord_t y, lv_color_t c)
{
    ...
    if(dsc->header.cf == LV_IMG_CF_TRUE_COLOR || dsc->header.cf == LV_IMG_CF_TRUE_COLOR_CHROMA_KEYED
#if (DLG_LVGL_CF == 1)
             || dsc->header.cf == LV_IMG_CF_RGB565
#endif /* DLG_LVGL_CF */
    ) {
        uint8_t px_size = lv_img_cf_get_px_size(dsc->header.cf) >> 3;
        uint32_t px     = dsc->header.w * y * px_size + x * px_size;
         ...
         *so the possible real width are 8 ,16, 24 ...*/
        uint32_t px = ((dsc->header.w + 7) >> 3) * y + x;
#if DLG_LVGL_CF_SUB_BYTE_SWAP
        buf_u8[px]  = buf_u8[px] & ~(1 << (7 - bit));
        buf_u8[px]  = buf_u8[px] | ((c.full & 0x1) << (7 - bit));
#else
        buf_u8[px]  = buf_u8[px] & ~(1 << bit);
        buf_u8[px]  = buf_u8[px] | ((c.full & 0x1) << bit);
#endif
    }
    else if(dsc->header.cf == LV_IMG_CF_INDEXED_2BIT) {
...
#else
        buf_u8[px] = buf_u8[px] & ~(3 << bit);
        buf_u8[px] = buf_u8[px] | ((c.full & 0x3) << bit);
#endif
    }
    else if(dsc->header.cf == LV_IMG_CF_INDEXED_4BIT) {
        buf_u8 += sizeof(lv_color32_t) * 16; /*Skip the palette*/
...
#else
        buf_u8[px]  = buf_u8[px] & ~(0xF << bit);
        buf_u8[px]  = buf_u8[px] | ((c.full & 0xF) << bit);
#endif
    }
    else if(dsc->header.cf == LV_IMG_CF_INDEXED_8BIT) {
        ...
        uint32_t px = (dsc->header.w * y + x)*4;
        uint32_t c_full_32 = lv_color_to32(c);
        buf_u8[px + 0]  = (c_full_32)       &0xFF;  /* blue */
        buf_u8[px + 1]  = (c_full_32>>8)    &0xFF;  /* green */
        buf_u8[px + 2]  = (c_full_32>>16)   &0xFF;  /* red */
    }
    else if(dsc->header.cf == LV_IMG_CF_ARGB4444) {
        uint32_t px = (dsc->header.w * y + x)*2;
        buf_u8[px]     = (uint8_t)(((((uint32_t)(c.ch.green * 15) / 0x3F) & 0xF) << 4) | (((uint32_t)(c.ch.blue * 15) / 0x1F) & 0xF));
        buf_u8[px + 1] = (buf_u8[px + 1] & 0xF0) | (uint8_t)((( (uint32_t)(c.ch.red   * 15) / 0x1F) & 0xF));
    }
    else if(dsc->header.cf == LV_IMG_CF_ARGB1555) {
        uint32_t px = (dsc->header.w * y + x)*2;
        buf_u8[px]     = (uint8_t)(((((uint32_t)(c.ch.green * 0x1F) / 0x3F) & 0x1F) << 5) | c.ch.blue);
        buf_u8[px + 1] = (buf_u8[px + 1] & 0x80) | (uint8_t)((c.ch.red   << 2) | ((((uint32_t)(c.ch.green * 0x1F) / 0x3F) & 0x1F) >> 3));
    }
    else if(dsc->header.cf == LV_IMG_CF_RGBA8888) {
        uint32_t px = (dsc->header.w * y + x)*4;
        uint32_t c_full_32 = lv_color_to32(c);
        buf_u8[px + 1]  = (c_full_32)       &0xFF;  /* blue */
        buf_u8[px + 2]  = (c_full_32>>8)    &0xFF;  /* green */
        buf_u8[px + 3]  = (c_full_32>>16)   &0xFF;  /* red */
    }
    else if(dsc->header.cf == LV_IMG_CF_RGBA4444) {
        uint32_t px = (dsc->header.w * y + x)*2;
        buf_u8[px]     = (buf_u8[px] & 0x0F) | (uint8_t)(((((uint32_t)(c.ch.blue * 15) / 0x1F) & 0xF) << 4));
        buf_u8[px + 1] = (uint8_t)(((((uint32_t)(c.ch.red  * 15) / 0x1F) & 0xF) << 4) | (((uint32_t)(c.ch.green * 15) / 0x1F) & 0xF));
    }
    else if(dsc->header.cf == LV_IMG_CF_RGBA5551) {
        uint32_t px = (dsc->header.w * y + x)*2;
        buf_u8[px]     = (buf_u8[px] & 0x01) | (uint8_t)(((((uint32_t)(c.ch.green * 0x1F) / 0x3F) & 0x1F) << 7) | (c.ch.blue  << 1));
        buf_u8[px + 1] = (uint8_t)((c.ch.red   << 3) | ((((uint32_t)(c.ch.green * 0x1F) / 0x3F) & 0x1F) >> 2));
    }
    else if(dsc->header.cf == LV_IMG_CF_RGB888) {
        uint32_t px = (dsc->header.w * y + x)*4;
        uint32_t c_full_32 = lv_color_to32(c);
         ...
         *so the possible real width are 2 ,4, 6 ...*/
        uint32_t px = dsc->header.w * y + x;
        buf_u8[px]  = (buf_u8[px] & 0xF0) | (c.full & 0xF);
    }
#endif /* DLG_LVGL_CF */
}
...
uint32_t lv_img_buf_get_img_size(lv_coord_t w, lv_coord_t h, lv_img_cf_t cf)
{
    switch(cf) {
        ...
        case LV_IMG_CF_RGB565:
            return LV_IMG_BUF_SIZE_RGB565(w, h);
        case LV_IMG_CF_RGB888:
            return LV_IMG_BUF_SIZE_RGB888(w, h);
        case LV_IMG_CF_ARGB8888:
            return LV_IMG_BUF_SIZE_ARGB8888(w, h);
        case LV_IMG_CF_RGBA8888:
            return LV_IMG_BUF_SIZE_RGBA8888(w, h);
        case LV_IMG_CF_ARGB4444:
            return LV_IMG_BUF_SIZE_RGBA4444(w, h);
        case LV_IMG_CF_RGBA4444:
            return LV_IMG_BUF_SIZE_ARGB4444(w, h);
        case LV_IMG_CF_ARGB1555:
            return LV_IMG_BUF_SIZE_ARGB1555(w, h);
        case LV_IMG_CF_RGBA5551:
            return LV_IMG_BUF_SIZE_RGBA5551(w, h);
        case LV_IMG_CF_AI44:
            return LV_IMG_BUF_SIZE_AI44(w, h);
#endif /* DLG_LVGL_CF */
        default:
            return 0;
    }
