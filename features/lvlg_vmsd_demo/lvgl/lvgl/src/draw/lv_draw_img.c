...
#if (DLG_LVGL_CF == 1)
/**
 * GPU color format support
 * @param cf a color format (`LV_IMG_CF_...`)
 * @return if the gpu supports the input color format
 */
bool lv_img_cf_use_gpu(lv_img_cf_t cf)
{
#if LV_USE_EXTERNAL_RENDERER == 1
        bool use_gpu = false;
...
#else
        return false;
#endif /* LV_USE_EXTERNAL_RENDERER == 1 */
}

/**
 * SW color format support
 * @param cf a color format (`LV_IMG_CF_...`)
 * @return if the SW supports the input color format
 */
bool lv_img_cf_use_sw(lv_img_cf_t cf)
{
    bool use_sw = false;
