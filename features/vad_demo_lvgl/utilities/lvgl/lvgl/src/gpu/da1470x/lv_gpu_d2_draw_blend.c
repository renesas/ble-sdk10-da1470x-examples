                                              ...
                                              const lv_opa_t * mask, lv_draw_mask_res_t mask_res)
{
    ...
    if(mask_res == LV_DRAW_MASK_RES_FULL_COVER) {
        if(disp->driver->gpu_fill_cb) {
            disp->driver->gpu_fill_cb(disp->driver, disp_buf, disp_w, draw_area, color, opa);
            return;
        }
    }
    /*Masked*/
    else if (lv_area_get_size(draw_area) > hor_res) {
        if(disp->driver->gpu_blit_cb && disp->driver->gpu_config_blit_cb) {
            lv_draw_img_dsc_t dsc;
            lv_draw_img_dsc_init(&dsc);
            dsc.recolor = color;

            if (disp->driver->gpu_config_blit_cb(disp->driver, &dsc, LV_IMG_CF_TRUE_COLOR, LV_IMG_CF_ALPHA_8BIT, true, false, true, true)) {
                if(disp->driver->gpu_wait_cb) disp->driver->gpu_wait_cb(disp->driver);
                disp->driver->gpu_blit_cb(disp->driver, disp_buf, draw_area, disp_w, (lv_color_t *)mask, draw_area, opa);
                return;
            }
        }
    }
                                          ...
                                          )
{
    ...
    /*Simple fill (maybe with opacity), no masking*/
    if(mask_res == LV_DRAW_MASK_RES_FULL_COVER) {
        if (!lv_img_cf_use_sw(cf) || lv_area_get_size(draw_area) > hor_res) {
            if(disp->driver->gpu_blit_cb && disp->driver->gpu_config_blit_cb) {
                lv_draw_img_dsc_t dsc;
                lv_draw_img_dsc_init(&dsc);

                if (disp->driver->gpu_config_blit_cb(disp->driver, &dsc, LV_IMG_CF_TRUE_COLOR, cf, false, false, true, false)) {
                    lv_area_t map_area_tmp;

                    lv_area_copy(&map_area_tmp, map_area);
                    lv_area_move(&map_area_tmp, -disp_area->x1, -disp_area->y1);

                    disp->driver->gpu_blit_cb(disp->driver, disp_buf, draw_area, disp_w, map_buf, &map_area_tmp, opa);
                    return;
                }
            }
        }
    ...
    if (!lv_img_cf_use_sw(cf) || (lv_area_get_size(draw_area) > DLG_LVGL_GPU_BLIT_MASK_SIZE_LIMIT)) {
        if(disp->driver->gpu_blit_with_mask_cb && disp->driver->gpu_config_blit_cb) {
            ...
            if (disp->driver->gpu_config_blit_cb(disp->driver, &dsc, LV_IMG_CF_TRUE_COLOR, cf, false, false, true, false)) {
                uint32_t *bmp_buf = lv_mem_buf_get(draw_area_w * draw_area_h * sizeof(uint32_t));
                if (!bmp_buf) {
                    return;
                }
                ...
                disp->driver->gpu_blit_with_mask_cb(disp->driver, disp_buf, draw_area, disp_w,
                    (lv_color_t *)map_buf, &map_area_rel, mask, bmp_buf, opa);
