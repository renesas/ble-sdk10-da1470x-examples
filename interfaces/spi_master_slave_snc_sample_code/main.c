 ...
 * @file main.c
 *
 * @brief SPI adapter demonstration example for SNC
 *
 ...
 * Task functions.
 */
OS_TASK_FUNCTION(spi_producer_task, pvParameters);
...
static OS_TASK_FUNCTION(system_init, pvParameters)
{
        ...
        /* Create a task that performs Tx operations over a serial bus */
        status = OS_TASK_CREATE("SPI PRODUCER",         /* The text name assigned to the task, for
        ...
        /* Create a task that performs Rx operations over a serial bus */
        status = OS_TASK_CREATE("SPI CONSUMER",        /* The text name assigned to the task, for
                                                          debug only; not used by the kernel. */
...
static void prvSetupHardware(void)
{
         ...
         */
        ad_spi_io_config(((ad_spi_controller_conf_t *)SPI_DEVICE_MASTER)->id,
                ((ad_spi_controller_conf_t *)SPI_DEVICE_MASTER)->io, AD_IO_CONF_OFF);

        ad_spi_io_config(((ad_spi_controller_conf_t *)SPI_DEVICE_SLAVE)->id,
                ((ad_spi_controller_conf_t *)SPI_DEVICE_SLAVE)->io, AD_IO_CONF_OFF);

#elif (dg_configI2C_ADAPTER && dg_configUSE_HW_I2C)
